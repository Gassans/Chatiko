...
async def get_channel_name(youtube, channel_id):
    try:
        ...
        logging.error(f"Ошибка YouTube API при получении имени канала: {e}")
    return None
        ...
        while chat.is_alive():
            for c in chat.get().sync_items():
                ...
                if author_id not in seen_users:
                    ...
                    user_name = await get_channel_name(youtube, author_id)
                    if user_name:
                        await send_message(f"Новый котэк на Ютубе❤️: {user_name}")
            await asyncio.sleep(1)  # минимальная задержка, чтобы не нагружать процессор
